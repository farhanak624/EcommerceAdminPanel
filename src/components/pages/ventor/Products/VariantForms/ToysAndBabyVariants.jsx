...
const ToysAndBabyVariants = ({ formData, setformData, errors }) => {
  ...
  const addDetail = (variantIndex) => {
    const updatedVariants = formData.variants.map((variant, vIndex) => {
      console.log(variant);
      if (vIndex === variantIndex) {
        const newDetail = { quantity: "", price: "" };
  ...
  return (
    <>
      <div className="flex justify-between mt-3">
        ...
        <button
          className="p-2 rounded-lg mb-2 text-black border border-gray-800 text-sm bg-bodycolur flex items-center"
          type="button"
          onClick={() => {
            addVariant();
          }}
        ...
        {formData.variants.map((variant, variantIndex) => (
          <div key={variantIndex}>
            ...
            <div className="flex gap-3 ">
              <input
                type="text"
                className="shadow appearance-none text-black border rounded w-[50%] py-2 px-3 bg-bodycolur   focus:outline-none focus:shadow-outline"
                defaultValue={""}
                name={variant.color}
                onChange={(e) =>
              ...
              {variant.images.map((image, imageIndex) => (
                <div key={imageIndex} className="flex relative mr-2">
                  <div className="grid-flow-row">
                    <div className="">
                      <input
                        className="outline-none border-none mr-3 bg-bodycolur mt-3 w-full h-10 p-1 rounded-lg"
                        type="file"
                        name={Color}
                        defaultValue={image}
                        onChange={(e) => {
...
export default ToysAndBabyVariants;
