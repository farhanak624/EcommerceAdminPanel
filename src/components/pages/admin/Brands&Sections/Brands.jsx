...
const Brands = () => {
  ...
  const sectionCategories = async (sectionId) => {
    dispatch(loadSpinner());
    try {
      const response = await getAdminSectionCategories(sectionId);
      ...
      response?.data.sectionData.categories.map((category) => {
  ...
  const brandModals = () => {
    setBrandEditModal(!brandEditModal);
  };
  const sectionModalCallback = () => {
    getProductSections();
    setSectionAddModal(!sectionAddModal);
  };
  ...
  return (
    <div>
      <div className="flex justify-between mt-2 w-full">
        ...
        {/* {brandEditModal && (
          <BrandModal callback={brandModals} editData={currentBrand}  role={"admin"}/>
        )} */}

        {sectionAddModal && <SectionModal callback={sectionModalCallback} />}
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6 pt-6  ">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="text-xl bg-navblue h-11 w-9 rounded-lg text-white"
            >
              +
            </button>
          </div>
      ...
      <div className="mt-6  overflow-y-auto">
        <div className="table-responsive p-3">
          <table className="table table-nowrap mb-0 w-full overflow-x-auto border">
